...
def ild(dpth,ppth):
    ...
    ild = cf.ild(sstm,"cor")
    np.save(dpth+'ild-cor', ild)
    np.savetxt(dpth+'ild-flat-cor.csv', ild.flatten(), delimiter=",")

    # compute ild only at causal loci
    hncsl = len(sstm.h.csl_snp)
    pncsl = len(sstm.p.csl_snp)
    csl_ild = ild[0:hncsl,0:pncsl]
    np.save(dpth+'csl-ild-cor', csl_ild)
    np.savetxt(dpth+'csl-ild-flat-cor.csv', csl_ild.flatten(), delimiter=",")

    # compute ild only at neutral loci
    ntl_ild = ild[hncsl:sstm.h.S,pncsl:sstm.p.S]
    np.save(dpth+'ntl-ild-cor', ntl_ild)
    np.savetxt(dpth+'ntl-ild-flat-cor.csv', ntl_ild.flatten(), delimiter=",")