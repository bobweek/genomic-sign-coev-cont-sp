...
Suppose \(f(x_1,x_2)\) is a function of two-dimensional space. For
simplicity, let's consider a rectangular subset \(\Gamma\) of
\(\mathbb R^2\) with periodic boundaries (ie., the surface of a torus).
Then we can compute the spatial average of \(f(x_1,x_2)\) via
...
Set \(\bar z_H(x_1,x_2)=\mathbb E[\bar Z_H(x_1,x_2)]\) and
\(\bar z_P(x_1,x_2)=\mathbb E[\bar Z_P(x_1,x_2)]\). Since the system
...
\end{equation} where \(\mathcal I\) captures the potential for
...
\(K_{\bar z_H\bar z_P}(x_1,x_2)\equiv0\).
