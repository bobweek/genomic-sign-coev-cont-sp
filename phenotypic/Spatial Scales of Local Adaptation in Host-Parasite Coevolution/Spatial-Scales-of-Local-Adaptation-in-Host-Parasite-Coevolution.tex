...
Julia (Bezanson et al. 2017) and can be found in the github repository
...
local adaptation for simulated populations. In particular, to simplify
comparison with analytical results, we fitted simulated trait data to
Matérn covariance functions with smoothness parameter \(\nu=1\)
separately for each species. This by fitting Matérn covariance functions
we were able to estimate spatial scales of phenotypic turnover in each
species for direct comparisons with analytical results. To quantify
local adaptation, we discretized space into \(100\times100\) subunits.

To compare characteristic scales of phenotypic turnover, we can consider
the distance at which the numerical SCF obtains the same correlation as
the Matern with smoothness one. Also implement a method to assess the
fit of a Matern covariance function to the numerically inferred SCF.
