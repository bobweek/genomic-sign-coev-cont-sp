...
This concludes the general description of our simulation model. Further details can be found in Appendix \ref{} (including details on spatially heterogeneous abiotic optima). Simulation model parameters are summarized in Table Y. Code for simulations were written in the language Julia [@Julia-2017] and can be found in the github repository \url{https://github.com/bobweek/continuous-space-coevolution}. In the next subsection, we describe our use of this simulation model to check the robustness of our analytical results.

### Assessing robustness of analytical results

To assess the robustness of our analytical results in face of broken assumptions, we quantified spatial scales of phenotypic turnover and local adaptation for simulated populations. In particular, to simplify comparison with analytical results, we fitted simulated trait data to Matérn covariance functions with smoothness parameter $\nu=1$ separately for each species. This by fitting Matérn covariance functions we were able to estimate spatial scales of phenotypic turnover in each species for direct comparisons with analytical results. To quantify local adaptation, we discretized space into $100\times100$ subunits. 

To compare characteristic scales of phenotypic turnover, we can consider the distance at which the numerical SCF obtains the same correlation as the Matern with smoothness one. Also implement a method to assess the fit of a Matern covariance function to the numerically inferred SCF.
