...
Vsol = solve([-2*(GH*AH+wH)*VH-2*GH*BH*CHP+GH/NH,
              ...
              -(GP*AP+wP)*CHP+GP*BP*VH-(GH*AH+wH)*CHP-GH*BH*VP],[VH,VP,CHP])

sp.print_latex(sp.simplify(Vsol[CHP].subs([(GH,G),(GP,G),(AH,A),(AP,A),(NH,N),(NP,N)])))

geomean = sp.sqrt(sp.simplify(Vsol[VH]*Vsol[VP]))
num = sp.simplify(sp.powdenest(Vsol[CHP], force=True))
rho = num/geomean
rho = sp.powdenest(rho.subs([(GH,G),(GP,G),(AH,A),(AP,A),(BH,B),(BP,B),(NH,N),(NP,N)]),force=True)
n, d = sp.fraction(rho)
sp.simplify(sp.powsimp(d,force=True))
sp.print_latex(sp.collect(n,[B,N]))
