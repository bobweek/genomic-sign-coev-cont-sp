...
where $\nu_{ij}=(\nu_i+\nu_j)/2$ and $\xi_{ij}=\min(\xi_i,\xi_j)$ and $\theta_{ij}$ is the collocated correlation between alleles at locus $i$ in species $H$ and locus $j$ in species $P$.
...
#
#
# generate two indep sets of samples at each location and investigate spurious spatial correlations since both are sp autocorr
#
#

#
#
# what is the expected corr of mean traits in space analytically? on a bounded region?
#
#
...
ftaa <- function(a){
  ...
  fit = sqrt(sum(sum(abs(cMv-pMv)))+sum((m-prpmn)^2))
...
for(i in 1:choose(n,2)){
  k = BetBet[i,1]
  l = BetBet[i,2]
  a1  = af[DirUnq[i,1:(K/2)]]
  a2  = af[DirUnq[i,(1+K/2):K]]
  a12 = af[DirDir[i,]]
  a0 = af[DirCmp[i,]]
  prpMat[k,l] = Cov(a1,a2,a12,a0)
  prpMat[l,k] = Cov(a1,a2,a12,a0)
...
require(fields) # for mle inference of matern cov pars
